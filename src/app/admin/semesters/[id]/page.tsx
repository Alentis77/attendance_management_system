import { getSemester, getAcademicYears } from '@/lib/data';
import { updateSemester } from '@/lib/actions';
import { redirect } from 'next/navigation';

export default async function EditSemesterPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    const [semester, years] = await Promise.all([
        getSemester(id),
        getAcademicYears()
    ]);

    if (!semester) {
        redirect('/admin/semesters');
    }

    const updateSemesterWithId = updateSemester.bind(null, id);

    return (
        <div>
            <h1 className="text-2xl font-bold mb-6 text-gray-900">Edit Semester</h1>

            <div className="bg-white p-6 rounded-lg shadow mb-8 max-w-md">
                <form action={updateSemesterWithId} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Name (e.g. Semester 1)</label>
                        <input
                            type="text"
                            name="name"
                            defaultValue={semester.name}
                            required
                            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 text-gray-900"
                        />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Academic Year</label>
                        <select
                            name="academicYear"
                            defaultValue={semester.academicYear?.toString()}
                            required
                            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2 text-gray-900"
                        >
                            <option value="">Select Year</option>
                            {years.map(y => <option key={y._id.toString()} value={y._id.toString()}>{y.year}</option>)}
                        </select>
                    </div>
                    <div className="flex justify-end gap-2">
                        <a href="/admin/semesters" className="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300">Cancel</a>
                        <button type="submit" className="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">Update</button>
                    </div>
                </form>
            </div>
        </div>
    );
}
